 ...
 *
 * Script to create Expansions of equal area
 * 
...
def parentObjectsClassification = "Tumor"
def numberOfExpansions = 2 //Must be an integer greater than 1 (e.g., 2, 3, 4, ...)
def directionOfDilation = "Inside"

def computePercentDifferenceBetweenRegions = true ////set to "true" if you want to compute it.
...
runExpansionsSameAreaParentsClassified(parentObjectsClassification,numberOfExpansions,directionOfDilation,computePercentDifferenceBetweenRegions)



//Main Function

def runExpansionsSameAreaParentsClassified(parentObjectsClassification, numberOfExpansions, directionOfDilation,computePercenDifferenceBetweenRegions ){

    // Check if numberOfDilations is not an integer or is 1
    if (!(numberOfExpansions instanceof Integer) || numberOfExpansions == 1) {
    ...
    if(parentObjects.size() == 0){
        println "ERROR: we do not have any classified parents. Please check parents classification"
        return
    }
    ...
    if(directionOfDilation == "Inside"){
        ...
        parentObjects.each{
            dilationsInsideParent(it,numberOfExpansions )
        }
        ...
        parentObjects.each{
            dilationsOutsideParent(it,numberOfExpansions )
        }
    }else{
